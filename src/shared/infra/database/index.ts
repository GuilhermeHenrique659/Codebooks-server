...
import { FilesSchema } from "../../../modules/files/infra/database/entities/FilesSchema";
...
export class DataBase {
    ...
    public getDataSource<T extends ObjectLiteral>(
        ...
        if (!useMock) {
            if (repositoryAdapter)
                return new repositoryAdapter(this.connection.getRepository<T>(Entity)) as unknown as IRepositoryAdapter<T>;
...
export const connection = new DataSource({
    ...
    entities: [userEntitySchema, PostEntitySchema, LikeEntitySchema, NotificationEntitySchema, FriendshipEntitySchema, FilesSchema],
});
